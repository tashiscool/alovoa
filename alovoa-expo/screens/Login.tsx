...
const Login = ({ route: _r, navigation: _n }: Props) => {
  ...
  const load = async () => {
    ...
    const mockMode = await Global.GetStorage(Global.STORAGE_MOCK_MODE);
    if (mockMode === Global.STORAGE_TRUE) {
      Global.enableMockMode();
    }
  ...
  const loginEmail = async () => {
    // Check for bypass/mock credentials first
    if (Global.tryMockLogin(email, password)) {
      hideDialog();
      console.log('[Login] Mock mode activated with bypass credentials');
      Global.ShowToast('Mock mode enabled - testing AURA features');
      await Global.SetStorage(Global.STORAGE_FIRSTNAME, 'TestUser');
      await Global.SetStorage(Global.STORAGE_PAGE, Global.INDEX_MAIN);
      await Global.SetStorage(Global.STORAGE_LOGIN_DATE, new Date().toISOString());
      await Global.SetStorage(Global.STORAGE_MOCK_MODE, Global.STORAGE_TRUE);
  ...
  async function emailSignInPress() {
    if (emailValid && password) {
      // Check for bypass credentials - skip captcha
      if (email === 'test@aura.dev' && password === 'aura123') {
        await loginEmail();
        return;
      }
      ...
      {!loading &&
        <View >
          <View style={{ minHeight: height }}>
            ...
            {/* Test Mode Button - for AURA feature testing */}
            {__DEV__ && (
              <View style={{ marginTop: 24 }}>
                <Button
                  ...
                  textColor="#10B981"
                >
                  Fill Test Credentials (AURA)
                </Button>
                <Text style={{ textAlign: 'center', marginTop: 8, fontSize: 12, color: colors.onSurfaceVariant }}>
                  test@aura.dev / aura123
                </Text>
              </View>
            )}
          </View>
